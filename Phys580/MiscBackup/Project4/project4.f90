...
!Project 4
!
!This program plots the wave function in three different situations.  First looking at
! the ground state and 1st Excited state of the particle in a box scenario, second
! the ground state and 1st Excited state of a harmonic oscillator, and third the
! ground state, 1st excited state, 2nd excited state and 3rd excited state of
! the Woods-Saxon potential.  These are all plotted with file names ending in HO:
! harmonic oscillator, WS: Woods-Saxon potential and no descriptive ending being
! the particle in a box scenario.  The ground, 1st, 2nd and 3rd excited states of the
! Woods-Saxon potential are plotted each for R = 2, 4, 6, 8, 10.
!
! 	Input Variables:
...
!
! 	Subroutines used: 
!		-'PEmatrix(a, R, V0, x, dx, N, L, PEmat)'
!			-Creates PE matrix for Woods-Saxon potential
...
Program project4
 ...
 do while (continue == 'y' .or. continue == 'Y')    
	...
	call WoodsSaxon(N, KEmat, dx, L, a, R, V0, HmatWS, HeValWS, HeVecWS)
	 ...
	 !call printnicematrix(N, N, KEeVec)
...
implicit none
 ...
 do i = 1, N !create file with eigenvector 	
	xi = -L + (i - 1)*dx
	!print*, 'KEeVal (', i, ')=', KEeVal(i)
...
implicit none
 ...
 do i = 1, N !create file with eigenvector 	
	xi = -L + (i - 1)*dx
	!print*, 'HeVal (', i, ')=', HeVal(i)
	write(3,*) xi, HeVec(i, 1)
	write(4,*) xi, HeVec(i, 2)
 enddo  	
