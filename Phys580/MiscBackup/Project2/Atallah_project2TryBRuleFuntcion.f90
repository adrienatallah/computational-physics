...
100	print *, ' '
	...
	!Do ix = 0, N
		...
		Do iy = 0, N
			...
			!Call BRule(fxyz, hz, N, zsum)  ! Call subroutine to calculate approximate integral using booles rule and get g(xy)
			!print *, 'zsum =  ', zsum
		
			gxy(iy) = BRule(fxyz, hz, N) !fill y integrand to send to booles rule to get f(x)
			print *, 'gxy(', iy, ')=', gxy(iy)	
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 double precision function BRule(farray, h, N)
implicit none
	
integer, intent(in) :: N	 
 double precision, intent(in) :: farray(0:1000), h
 	!Input variables 
 double precision :: k, total, I(0:1000)
integer :: j	
