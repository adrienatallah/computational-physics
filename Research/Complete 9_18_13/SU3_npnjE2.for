!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Original SU3_npnjl.for with added subroutines DNSQE.f and DQK15I.f
!	
!  QDAGI is replaced by DQK15I
!  DNEQNF is replaced by DNSQE
!  	
! Subroutine dependencies: 
! 
! DNSQE -> DNSQ (DQK15I) -> D1MACH -> XERMSG (DNSQE already calls). 
			! D1MPYQ.
...
!BEGIN MAIN***************************************************************************************
	...
	Integer IOPT,NPRINT,INFO,LWA,NWRITE !added for DNSQE
	...
	DOUBLE PRECISION TOL,FVEC(N),WA(180) !DNSQE
	DOUBLE PRECISION JAC,DENORM,D1MACH  !DNSQE
	
        INTEGER INF !DQK15I
        DOUBLE PRECISION BOUN,ABSERR,RESABS,RESASC,A,B !DQK15I
 
        EXTERNAL FCN,C_ss,C_uu !,DNEQNF
	...
	Common/param_DQk15I/BOUN,INF,ABSERR,RESABS,RESASC,A,B
...
!50    continue
      ...
      CALL DQK15I(C_ss,BOUN,INF,A,B,qq_s,ABSERR,RESABS,RESASC)

      If (i.eq.1)qq_sshift=qq_s
30    continue

      !CALL DQDAGI (C_uu,BOUND,INTERV,ERRABS,ERREL,qq_u,ERREST)
       CALL DQK15I(C_uu,BOUN,INF,A,B,qq_u,ABSERR,RESABS,RESASC)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	phi3=fi3
      ...
      CALL DQK15I(Fun_Omu,BOUN,INF,A,B,xomu,ABSERR,RESABS,RESASC)
      
      !CALL DQDAGI (Fun_Oms,BOUND,INTERV,ERRABS,ERREL,xoms,ERREST)
      CALL DQK15I(Fun_Oms,BOUN,INF,A,B,xoms,ABSERR,RESABS,RESASC)
      
      !CALL DQDAGI (S_u,BOUND,INTERV,ERRABS,ERREL,int_u,ERREST)
      CALL DQK15I(S_u,BOUN,INF,A,B,int_u,ABSERR,RESABS,RESASC)
      
      !CALL DQDAGI (S_s,BOUND,INTERV,ERRABS,ERREL,int_s,ERREST)
      CALL DQK15I(S_s,BOUN,INF,A,B,int_s,ABSERR,RESABS,RESASC)
...
!	pause
      ...
      CALL DQK15I(Der1_Om_u,BOUN,INF,A,B,x1u_dphi3,ABSERR,RESABS,RESASC)
      
      !CALL DQDAGI (Der1_Om_s,BOUND,INTERV,ERRABS,ERREL,x1s_dphi3,ERREST)
      CALL DQK15I(Der1_Om_s,BOUN,INF,A,B,x1s_dphi3,ABSERR,RESABS,RESASC)	      
...
!Calculo de la densidad    
	...
	Valor=(-2.d0*N_c*xT/pi2)
      ...
      CALL DQK15I(Derivu_mu,BOUN,INF,A,B,xdensiu,ABSERR,RESABS,RESASC)
      
      !CALL DQDAGI(Derivs_mu,BOUND,INTERV,ERRABS,ERREL,xdensis,ERREST)
      CALL DQK15I(Derivs_mu,BOUN,INF,A,B,xdensis,ABSERR,RESABS,RESASC)
...
!******************************************************
! for DNSQE, the format should be:
! 	
!	SUBROUTINE FCN (N,FVEC,IFLAG)
! 	INTEGER N, IFLAG
...
!********************************************
        ...
        Implicit none
        INTEGER N,IFLAG,INTERV,nreg,poly
        Real*8 X(3),FVEC(3),sigu,sigs,phi3,campu,camps,sigumax,sigsmax 
...
!	pause
	...
	FVEC(1)=sigu+G_s*Ese_u+H*Ese_u*Ese_s/2.d0 !changed F(1)->FVEC(1)
	FVEC(2)=sigs+G_s*Ese_s+H*Ese_u**2/2.d0
	FVEC(3)=0.0d0
	else
	FVEC(1)=sigu/G_s+Ese_u+H*Ese_u*Ese_s/(2.d0*G_s)
!	FVEC(1)=sigu+G_s*Ese_u+H*Ese_u*Ese_s/2.d0
	FVEC(2)=sigs+G_s*Ese_s+H*Ese_u**2/2.d0
	FVEC(3)=dOmega_dphi3(sigu,sigs,phi3)
      endif
!      write(*,*)''
